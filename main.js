...
async function saveAndShowFinalResult() {
    ...
    let compHtml = passRate >= PASSING_BORDER 
        ...
        : `<p class="warning-text">WARNING! 合格ボーダーまであと ${PASSING_BORDER - passRate}%！</p>`;
...
async function drawHistoryChart() {
    ...
    lineChartInstance = new Chart(ctx, {
        ...
        data: { 
            labels: labels, 
            datasets: [ 
                { label: '正答率 (%)', data: dataPoints, borderColor: '#00f3ff', backgroundColor: 'rgba(0, 243, 255, 0.1)', borderWidth: 3, fill: true }, 
...
async function openStatsScreen() {
    ...
    try {
        ...
        if (snapshot.empty) { 
            logList.innerHTML = `<p style="text-align: center;">まだ戦歴がないぞ！</p>`; 
            return; 
        }
        ...
        snapshot.forEach(doc => {
            ...
            logList.innerHTML += `
                <div class="log-item">
                    ...
                    <span>${data.mode}問</span>
                    <span class="log-pct ${data.percentage >= PASSING_BORDER ? "" : "low"}">${data.percentage}%</span>
                </div>
            `;
        });
        
        document.getElementById("weekly-count").innerHTML = `${wTotal}`; 
        document.getElementById("weekly-rate").innerHTML = `${wTotal > 0 ? Math.round((wCorrect / wTotal) * 100) : 0}`;
        document.getElementById("monthly-count").innerHTML = `${mTotal}`; 
        document.getElementById("monthly-rate").innerHTML = `${mTotal > 0 ? Math.round((mCorrect / mTotal) * 100) : 0}`;
        
    } catch (e) { 
        document.getElementById("log-list").innerHTML = `<p>データ取得エラー！</p>`; 
    }
}
